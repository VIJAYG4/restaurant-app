...
class RestaurantList extends Component {
  ...
  componentDidMount() {
    this.getRestaurantsFromApi("New York");
  }
  ...
  renderEmptyState() {
    return (
      <h2 className="heading-tertiary">
        `Hang tight! We are working on getting you the list of best brunch spots
  ...
  renderRestaurantInfo() {
    ...
    if (this.props.isFavorite && this.props.favResults.length) {
      RestaurantList = this.props.favResults.map(favResultItem => (
        <DisplayResult
          body={favResultItem}
          favResults={this.props.favResults}
        />
      ));
    } else {
      RestaurantList = this.state.results.map(result => (
        <DisplayResult body={result} favResults={this.props.favResults} />
      ));
    }

    return <div className="RestuarantList__gallery">{RestaurantList}</div>;
  }

  render() {
    console.log("This is favResults : ", this.props.favResults);
    return (
...
class DisplayResult extends Component {
  ...
  addFavorite(result) {
    ...
    this.props.favResults.push(result);
  }
  render() {
    console.log(
      "This is favResults in DisplayResults: ",
      this.props.favResults
    );
    const result = this.props.body;
    return (
      <div className="RestaurantInfo" key={result.id}>
        ...
        />
        <button key={result.id} onClick={() => this.addFavorite(result)}>
          {this.state.isFavorite ? "You already Favorited it" : "Favorite it!"}
        </button>
      </div>
    );
  }
